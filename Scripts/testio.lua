...
local tsInst = TargetSelector(2000, 1, myHero, true, menuInst, true)
...
ShulepinAIO_Ezreal = classInstance()

function ShulepinAIO_Ezreal:__init()
        self.menu = menuInst.addItem(SubMenu.new("Ezreal"))

        self.menu.addItem(MenuSeparator.new("   General Features", true))
        self.menu_combo = self.menu.addItem(SubMenu.new("Combo"))
        self.menu_combo.addItem(MenuSeparator.new("   Spell Settings", true))
        ...
        self.menu.addItem(MenuSeparator.new("   Advandced Features", true))
        self.menu_keys = self.menu.addItem(SubMenu.new("Keybinds"))
        self.menu_keys_combo = self.menu_keys.addItem(MenuKeyBind.new("Combo Key", 32))
        self.menu_keys_harass = self.menu_keys.addItem(MenuKeyBind.new("Harass Key", str.byte("C")))


        self.Q = Spell(_Q, 1150)
        self.Q:SetSkillShot(0.25, 2000, 85, true)
        self.W = Spell(_W, 1000)
        self.W:SetSkillShot(0.25, 1600, 80, false)

        self.sheenBuff = false
        self.sheenTime = 0
        ...
        DelayAction(function() print("[ShulepinAIO] Ezreal Loaded! (v1.0)") end, 1)
end

function ShulepinAIO_Ezreal:Orbwalk(target)
        if target then
...
function ShulepinAIO_Ezreal:CastQ(target)
        if self.Q:IsReady() and IsValidTarget(target, self.Q.range) then
                self.Q:Cast(target)
        end
end

function ShulepinAIO_Ezreal:CastW(target)
        if self.W:IsReady() and IsValidTarget(target, self.W.range) then
                self.W:Cast(target)
        end
end

function ShulepinAIO_Ezreal:Combo(target)
        if target and target ~= 0 then
                if HasSheen() then
                        if not self.sheenBuff and self.sheenTime + 1.5 < GetTimeGame() and GetDistance(GetAIHero(target)) <= GetTrueAttackRange() then
                                if self.menu_combo_q.getValue() then self:CastQ(target) end 
                                if self.menu_combo_w.getValue() then self:CastW(target) end
                        elseif GetDistance(GetAIHero(target)) > GetTrueAttackRange() then
                                if self.menu_combo_q.getValue() then self:CastQ(target) end 
                                if self.menu_combo_w.getValue() then self:CastW(target) end
                        end
                else
...
function ShulepinAIO_Ezreal:OnUpdate()
        local target = tsInst:GetTarget()
        local targetOrb = tsInst:GetTarget(GetTrueAttackRange())

        DisableOrb({self.menu_keys_combo, self.menu_keys_harass})

        if self.menu_keys_combo.getValue() then
                self:Orbwalk(targetOrb)
                self:Combo(target)
        end
end

function ShulepinAIO_Ezreal:OnUpdateBuff(unit, buff, stacks)
...
function ShulepinAIO_Ezreal:OnRemoveBuff(unit, buff)
        if unit.IsMe and buff.Name == "sheen" then
                self.sheenBuff = false
                self.sheenTime = GetTimeGame()
        end
end
...
ShulepinAIO_Jayce = classInstance()

function ShulepinAIO_Jayce:__init()
        self.menu = menuInst.addItem(SubMenu.new("Jayce"))

        self.menu.addItem(MenuSeparator.new("   General Features", true))
        self.menu_combo = self.menu.addItem(SubMenu.new("Combo"))
        self.menu_combo.addItem(MenuSeparator.new("   Spell Settings", true))
        self.menu_combo_q = self.menu_combo.addItem(MenuBool.new("wip", true))
        --self.menu_combo_w = self.menu_combo.addItem(MenuBool.new("Use W", true))
        ...
        DelayAction(function() print("[ShulepinAIO] Jayce Loaded! (v1.0)") end, 1)
end
...
function ShulepinAIO_Jayce:Orbwalk(target)
        if target then
                local attackRange = GetTrueAttackRange()
...
function ShulepinAIO_Jayce:OnUpdate()
        self:CD()
...
ShulepinAIO_BaseUlt = classInstance()

function ShulepinAIO_BaseUlt:__init()
        self.menu = menuInst.addItem(SubMenu.new("BaseUlt"))
        self.useBaseUlt = self.menu.addItem(MenuBool.new("Use BaseUlt", true))

        self.recallData = {}

        for i = 1, #GetEnemyHeroes() do
                self.recallData[#self.recallData + 1] = { hero = GetEnemyHeroes()[i], isRecalling = false, time = 0, timeToHit = 0 }
        end
                ...
                ["Ezreal"] = { --wip
                        ...
                        damage = function(target)
                                ...
                                for i, unit in pairs(GetEnemyHeroes()) do
                                        if unit ~= 0 then
                                                local unit = GetAIHero(unit)
                                                
                                                if unit and not unit.IsDead and unit.IsVisible then
                                                        local pointSegment, pointLine, isOnSegment = VectorPointProjectionOnLineSegment(Vector(myHero), self.basePos, Vector(unit))

                                                        if isOnSegment and Vector(unit):DistanceTo(Vector(pointSegment.x, GetAIHero(target).y, pointSegment.z)) <= GetOverrideCollisionRadius(myHero.Addr) + 150 then
                                                                count = count + 1
                                                        end
                                                end
                                        end
                                ...
                                local lvl = GetSpellLevel(myHero.Addr, _R)
                                if lvl > 0 then
                                        local red = count < 7 and (1 - 0.1 * count) or 0.3
                                        local dmg = red * (200 + 150 * lvl + myHero.BonusDmg + 0.9 * myHero.MagicDmg * myHero.MagicDmgPercent)
        ...
        self.basePos = myHero.Team == 100 and Vector(396, 183, 530) or Vector(14306, 171, 14458)

        Callback.Add("Update", function() self:OnUpdate() end)

        DelayAction(function() print("[ShulepinAIO] BaseUlt Loaded! (v1.0)") end, 1)
end

function ShulepinAIO_BaseUlt:OnUpdate()
        if not self.useBaseUlt.getValue() then return end

        for i = 1, #self.recallData do
                local data = self.recallData[i]
                
                if data then
                        local hero = data.hero
                        if hero == nil then return end

                        if IsRecall(hero) then
                                if data.isRecalling then
                                        if self.spellData[myHero.CharName] then
                                                local spellData = self.spellData[myHero.CharName]
                                                
                                                if CanCast(_R) and 70 * spellData.damage(hero) / 100 > GetHealthPoint(hero) then
                                                        local distance = GetDistance(self.basePos)
                                                        local speed = spellData.speed

                                                        data.timeToHit = distance / speed + spellData.delay

                                                        if GetTimeGame() + data.timeToHit >= data.time and GetTimeGame() + data.timeToHit < data.time + 0.25 then
                                                                CastSpellToPos(self.basePos.x, self.basePos.z, 3)
                                                        end
                                                end
                                        end
                                else
                                        data.isRecalling = true
                                        data.time = GetTimeGame() + 8
                                end
                        else
...
Callback.Add("Load", function()
        ...
        local baseUltHeroes = { "Ezreal" }
        if table.contains(baseUltHeroes, myHero.CharName) then
                ShulepinAIO_BaseUlt()
        end
end)
